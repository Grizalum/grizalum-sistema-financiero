 ...
 * Sistema de notificaciones integrado con Panel Admin
 * ================================================================
 */
...
class GrizalumNotificacionesPremium {
    ...
    inicializar() {
        ...
        this.crearAPI();
        console.log('✅ Sistema de Notificaciones Premium v2.6 cargado');
    }
    ...
    crearPanel() {
        ...
        panel.innerHTML = `
            <div class="notification-header">
                ...
                <span id="empresaDisplay">Selecciona una empresa</span>
            </div>
    ...
    detectarEmpresa() {
        ...
        if (selector) {
            ...
            for (const variante of variantes) {
                const activa = selector.querySelector(variante);
                if (activa?.textContent?.trim()) {
        ...
        if (nombreEmpresa && nombreEmpresa.length > 0) {
            const nombreLimpio = nombreEmpresa.replace(/[^\w\s]/g, '').trim().replace(/\s+/g, ' ');
            const empresaKey = nombreLimpio.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').substring(0, 50);
            
            if (empresaKey !== this.empresaActual) {
                this.empresaActual = empresaKey;
                console.log(`🏢 Empresa: ${nombreLimpio} → ${empresaKey}`);
    ...
    renderizar(filtro = 'todas') {
        ...
        lista.innerHTML = filtradas.map(notif => {
            ...
            const prefijo = notif.esAdmin ? '[ADMIN] ' : '';
            
            return `
                <div class="notification-item ${!notif.leida ? 'unread' : ''} ${claseAdmin}" data-id="${notif.id}">
                    <div class="notification-icon" style="background: ${categoria.color}">
                        <i class="${categoria.icono}"></i>
                    </div>
                    <div class="notification-content">
                        <div class="notification-title">${prefijo}${notif.titulo}</div>
        ...
        window.GrizalumNotificacionesPremium = {
            ...
            recibirDelAdmin: (empresaId, titulo, mensaje, tipo = 'admin') => {
                try {
                    ...
                    const config = mapeo[tipo] || mapeo['admin'];
                    const id = instanciaNotificaciones.crearNotificacion({
                        categoria: config.cat,
                        prioridad: config.prio,
                        titulo: titulo,
                        mensaje: mensaje,
                        esAdmin: true
                    });
                    ...
                    if (empresaOriginal === empresaId) {
                        setTimeout(() => instanciaNotificaciones.cargarNotificaciones(), 100);
                    }
...
setTimeout(() => {
    if (window.adminEmpresas && window.GrizalumNotificacionesPremium) {
        console.log('🎉 Conexión: Admin ↔ Notificaciones');
    }
}, 2000);
