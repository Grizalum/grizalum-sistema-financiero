/**
 * ================================================================
 * GRIZALUM NOTIFICACIONES PREMIUM v2.6
 * ================================================================
 */
...
class GrizalumNotificacionesPremium {
    ...
    inicializar() {
        ...
        this.crearAPI();
        console.log('✅ Notificaciones Premium v2.6');
    }
    ...
    detectarEmpresa() {
        ...
        const selector = document.getElementById('companySelector');
        if (selector) {
            const variantes = ['.active', '[data-selected="true"]', '.selected', 'button.active'];
            for (const v of variantes) {
                const activa = selector.querySelector(v);
                if (activa?.textContent?.trim()) { nombreEmpresa = activa.textContent.trim(); break; }
            }
        }
        if (nombreEmpresa && nombreEmpresa.length > 0) {
            const empresaKey = nombreEmpresa.replace(/[^\w\s]/g, '').trim().replace(/\s+/g, ' ').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').substring(0, 50);
    ...
    marcarLeida(id) {
        ...
        if (notif) { notif.leida = true; this.actualizarContador(); this.renderizar(); }
    }
    ...
    actualizarContador() {
        ...
        if (badge) { badge.textContent = noLeidas; badge.classList.toggle('active', noLeidas > 0); }
    ...
    renderizar(filtro = 'todas') {
        ...
        if (filtradas.length === 0) { lista.innerHTML = '<div class="empty-state">📭 Sin notificaciones</div>'; return; }
        ...
        document.querySelectorAll('.notification-item').forEach(item => item.addEventListener('click', () => this.marcarLeida(item.dataset.id)));
    }
    ...
    cargarNotificaciones() { this.actualizarContador(); this.renderizar(); }
        ...
        window.GrizalumNotificacionesPremium = {
            ...
            recibirDelAdmin: (empresaId, titulo, mensaje, tipo = 'admin') => {
                try {
                    ...
                    const orig = instanciaNotificaciones.empresaActual;
                    instanciaNotificaciones.empresaActual = empresaId;
                    const map = { admin: { cat: 'SISTEMA', prio: 'alta' }, info: { cat: 'SISTEMA', prio: 'media' }, warning: { cat: 'VENCIMIENTO', prio: 'alta' }, urgent: { cat: 'FINANCIERO', prio: 'critica' }, success: { cat: 'OPORTUNIDAD', prio: 'media' } };
                    const cfg = map[tipo] || map['admin'];
                    const id = instanciaNotificaciones.crearNotificacion({ categoria: cfg.cat, prioridad: cfg.prio, titulo, mensaje, esAdmin: true });
                    instanciaNotificaciones.empresaActual = orig;
                    if (orig === empresaId) setTimeout(() => instanciaNotificaciones.cargarNotificaciones(), 100);
                    return id;
...
setTimeout(() => { if (window.adminEmpresas && window.GrizalumNotificacionesPremium) console.log('🎉 Admin ↔ Notif'); }, 2000);
